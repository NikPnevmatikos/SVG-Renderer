...
| D9 | 2026-09-04 | React DOM support ships inside `svg-renderer` as the subpath `svg-renderer/web` (renderer + viewer with the same API), not as a separate package; `react-native` and `react-native-svg` become optional peers. | One install and one API for apps that have both a native app and a web app; the core and every viewer concept were already platform-neutral, only gestures and the camera transform differ. |
